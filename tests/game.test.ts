...
describe('game state flow', () => {
  it('explodes and reaches 0m when any die rolls zero', () => {
    ...
    const next = simulateLaunch(state, new SequenceRng([0, 0.6, 0.6, 0.6, 0.6]));
  ...
  it('uses linear height: score 5 reaches 5m', () => {
    const state = createInitialState(1);
    const next = simulateLaunch(state, new SequenceRng([0.6, 0.6, 0.6, 0.6, 0.6, 0.1, 0.2, 0.3]));
  ...
  it('awards one card pick after every successful launch', () => {
    const state = createInitialState(1);
    const first = simulateLaunch(state, new SequenceRng([0.6, 0.6, 0.6, 0.6, 0.6, 0.1, 0.2, 0.3]));
    const chosen = chooseCard(first, 0, new SequenceRng([0.1, 0.2, 0.3]));
    const second = simulateLaunch(chosen, new SequenceRng([0.6, 0.6, 0.6, 0.6, 0.6, 0.1, 0.2, 0.3]));
  ...
  it('awards milestone meta once per run', () => {
    ...
    const third = simulateLaunch(restarted, new SequenceRng([0.6, 0.6, 0.6, 0.6, 0.6, 0.1, 0.2, 0.3]));
  ...
  it('claims exactly one meta currency per bankruptcy', () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      state = simulateLaunch(state, new SequenceRng([0, 0, 0, 0, 0]));
    }
